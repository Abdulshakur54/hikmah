<?php
    ...
    class ExamAlert {
        ...
        public function send($receiver_id, $title, $message, $requery = false){
                ...
                if($requery){
                    if($this->_queryDeletePrepared){
                        $this->_db2->requery([$receiver_id]);
                         ...
                         $this->_db2->query('delete from ex_alert where receiver_id =? order by id asc limit 1',[$receiver_id]); //delete one alert row from table
                         $this->_queryDeletePrepared = true;
                    }
                   
                }else{
                     $this->_db->query('delete from ex_alert where receiver_id =? order by id asc limit 1',[$receiver_id]); //delete one alert row from table
                }
            ...
            if($requery){
                if($this->_queryInsertPrepared){
                    ...
                    $this->_db3->query('insert into ex_alert(receiver_id,title,message) values(?,?,?)',[$receiver_id, $title, $message]);
                    $this->_queryInsertPrepared = true;
                }
                
            }else{
                $this->_db->query('insert into ex_alert(receiver_id,title,message) values(?,?,?)',[$receiver_id, $title, $message]);
            }
        ...
        public function sendToRank(int $receiver_rank, string $title, string $message,$condition=null){
        ...
        public function getMyAlerts($receiver_id){
            $this->_db->query('select * from ex_alert where receiver_id = ? order by id desc',[$receiver_id]);
        ...
        public function delete($id){
            $this->_db->query('delete from ex_alert where id=?',[$id]);
        }
        ...
        public function getCount($receiver_id,$requery) :int{
            if($requery){
                 if($this->_querySelectedPrepared){
                    ...
                    $this->_db->query('select count(id) as counter from ex_alert where receiver_id = ?',[$receiver_id]);
        ...
        public function getUnseenCount($receiver_id) :int{
            $this->_db->query('select count(id) as counter from ex_alert where receiver_id = ? and seen = ?',[$receiver_id, false]);
            return $this->_db->one_result()->counter;
        }
        
        public function seen($receiver_id){
            $this->_db->query('update ex_alert set seen = ? where receiver_id=?',[true,$receiver_id]);
        }
