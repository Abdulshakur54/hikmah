    ...
    <div class="card">
        <div class="card-body">
            ...
            if (!empty($stds)) {
                ...
                echo '<table class="table table-striped table-bordered nowrap responsive" id="studentsTable"><thead><th>S/N</th><th>Student ID</th><th>Fullname</th></thead><tbody>';
                foreach ($stds as $std) {
                    echo '<tr><td></td><td>' . Utility::escape($std->std_id) . '</td><td>' . Utility::formatName(Utility::escape($std->fname), Utility::escape($std->oname), Utility::escape($std->lname)) . '</td></tr>';
                }
                echo '</tbody></table>';
