...
if (Input::submitted() && Token::check(Input::get('token'))) {
    ...
    if ($val->check($formvalues) && $val->checkFile($fileValues) && Utility::noScript(Input::get('address'))) {
        ...
        if (Utility::equals($pin, $agg->lookUp('token', 'token', 'token,=,' . $pin))) { //confirm the token
            $staff_id = 'S' . Staff::genId(); //generates the staff id
            ...
            $ext = $file->extension();
            $pictureName = $staff_id . '.' . $ext;
            $permission = Permission::getDefaultPermissions($rank);
            $sql1 = 'insert into ' . Config::get('users/table_name1') . '(' . Config::get('users/username_column1') . ', ' . Config::get('users/password_column') . ',title,fname,lname,oname,rank,sch_abbr,phone,email,permission,address,picture) values(?,?,?,?,?,?,?,?,?,?,?,?,?)';
            $vals1 = [$staff_id, password_hash($password, PASSWORD_DEFAULT), $title, $fname, $lname, $oname, $rank, $sch_abbr, Utility::escape(Input::get('phone')), $email, $permission, $address, $pictureName]; //null values should be changed for permission and email
            $sql2 = 'insert into account(receiver,no,bank,salary) values(?,?,?,?)';
            $vals2 = [$staff_id, Utility::escape(Input::get('account')), Utility::escape(Input::get('bank')), $salary];
            ...
            if ($db->trans_query([[$sql1, $vals1], [$sql2, $vals2]])) { //performs a transaction which consist of 2 queries
                ...
                $req->send($staff_id, 3, 'Please, confirm a request of &#8358;' . $salary . ' as salary for ' . $title . ' ' . Utility::formatName($fname, $oname, $lname), 1);
                //notify the APM(s)
                $notMessage = 'This is to inform you that ' . $title . ' ' . Utility::formatName($fname, $oname, $lname) . ' has successfully registered as ' . formatPositionMsg($rank) . '.<br>A request has been sent to the accountant to confirm his salary of &#8358;' . $salary;
                ...
                Session::set_flash('new_user', '<div>Thanks for Registering. You can now Login to your account</div><div>Your Username is <strong>' . $staff_id . '</strong><br><em>Copy and save it</em></div>');
...
{
    global $staff;
    return 'a ' . $staff->getFullPosition($rank);
}
