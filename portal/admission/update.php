<?php
	...
	if(Input::submitted() && Token::check(Input::get('token'))){
		...
		if($val->check($values)){
			if(!$adm->update(['fname'=>Input::get('fname'), 'lname'=>Input::get('lname'), 'oname'=>Input::get('oname')])){
				echo 'update failed';
			}{
				Session::set_flash('update','<span class="success">Your Details have been Successfully Updated<br></span>');
				Redirect::home('index.php',5);
			}
		}else{
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<?php echo Utility::escape($url->to('styles/update.css',5))?>" />
</head>
<body>
    <main>
        <?php
