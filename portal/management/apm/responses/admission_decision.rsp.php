            ...
            foreach ($idArr as $id) {
                if (!$start) {
                    ...
                    $body = '<div style="font-size: 19px; padding: 15px">Congratulations! You have been offered admission to <em>' . School::getLevelName($appSchool, $appLevel) . ', ' . School::getFullName($appSchool) . '</em></div>' .
                        '<div>Kindly click on this link <a href="' . $url->to('dashboard.php?page=admission/admission_status.php', 0) . '"> My Response to your offer</a> to respond to our offer.</div>';
                    ...
                    $body = '<div><div>Congratulations! You have been offered admission to <em>' . School::getLevelName($appSchool, $appLevel) . ', ' . School::getFullName($appSchool) . '</em></div>' .
                        '<div>Kindly click on this link <a class="text-primary" onclick="getPage(\'' . $url->to('dashboard.php?page=admission/admission_status.php', 0) . '\')" href="#"> My Response to your offer</a> to respond to our offer.</div>';
