...
</div> <input type="hidden" value="<?php echo $term ?>" name="term" id="term" />
</div> <input type="hidden" value="<?php echo Token::generate() ?>" name="token" id="token" />
<script>
    function submitClass() {
        let cls = _('class').value;
        getPage('management/hos/performance_summary.php?class_id=' + cls +
            '&token=' +
            _('token').value);
    }
    ...
    function changeClass() {
        ...
        if (term.length > 0) {
            getPage('management/hos/performance_summary.php?class_id=' + cls + '&term=' + term +
                '&token=' +
                _('token').value);
        }
