    ...
    //initializations
     ...
     if(Input::submitted() && Token::check(Input::get('token'))){
        ...
        if($category === 'updatesalary'){
            ...
            $req->delRequest($receiver, RequestCategory::UPDATE_SALARY);// 1 as a parameter here shows the category is salary
            $account->updateSalary($receiver,$salary); //update salary
            //send a confirmation request to the accountant
            $req->send($receiver, 3,'Please, confirm a request of &#8358;'.$salary.' as salary for '.$name, RequestCategory::UPDATE_SALARY);
            echo json_encode(['success'=>true,'token'=>Token::generate(),'id'=>$id,'category'=>$category,'receiver'=>$receiver]);
         }else{
   
            //send a confirmation request to the accountant
            if($status = $req->send($receiver, 3,'Please, confirm a request of &#8358;'.$salary.' as salary for '.$name, RequestCategory::UPDATE_SALARY)){
                if($status === 1){
