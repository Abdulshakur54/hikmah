     ...
     if(Input::submitted() && Token::check(Input::get('token'))){
         ...
         if($db->row_count() > 0){
            ...
            while($run){
                ...
                if($db->row_count() === 0){
                    //add token to table
                    $db->query('insert into token (token,owner,salary,sch_abbr,level,pro_rank,asst,added_by) values(?,?,?,?,?,?,?,?)',[$createdToken,$name,$salary,$sch_abbr,null,$rank,$asst,1]); //the 1 at the end shows it is added by the director
                    $run = false;
                }
            }
