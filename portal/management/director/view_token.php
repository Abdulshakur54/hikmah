...
<body>
    <main>
        ...
        <?php
            ...
            <table>
                <thead>
                    <tr><th>Name</th><th>Pin</th><th>Position</th><th>Action</th></tr>
                </thead>
                <tbody>
                    <?php 
                    $res=$db->get_result();
                    foreach($res as $val){
                        echo '<tr id="row'.$val->id.'"><td>'.Utility::escape(ucwords($val->owner)).'</td><td>'.Utility::escape($val->token).
                             '</td><td>'.$dir->getPosition($val->pro_rank).'</td><td class="deleteData" onclick="deleteToken('.$val->id.')">Delete</td><tr>';
                    }
                    ?>
                </tbody>
            </table> <?php
