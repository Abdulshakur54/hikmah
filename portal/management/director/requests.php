            ...
            ?>
                    ...
                    <div class="card border border-1 rounded mb-3" id="row<?php echo $rqst->id ?>">
                        ...
                        <div class="card-footer d-flex justify-content-end border-0" style="background-color:#fff;">
                            <?php
                            $footer = '<button class="btn btn-success btn-sm mr-3" onclick="accept(' . $rqst->id . ',\'' . $rqst->requester_id . '\',' . $rqst->category . ')" id="accept_' . $rqst->id . '">Accept</button><span id="ld_loader_' . $rqst->id . '"></span><button class="btn btn-danger btn-sm" onclick="decline(' . $rqst->id . ',\'' . $rqst->requester_id . '\',' . $rqst->category . ')" id="decline_' . $rqst->id . '">Decline</button>';
                            if ($rqst->type == 1) { //salary payment request
                                $footer = '<button class="btn btn-info btn-sm mr-3" onclick="getPage(\'management/director/confirm_salary_payment.php?reqId=' . $rqst->id . '\')" >Details</button>' . $footer;
                            }
                            echo $footer;
                            ?>
                        </div>
