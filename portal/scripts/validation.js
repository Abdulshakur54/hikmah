function validate(elementId,rule={},msgDivId){
    ...
    for(let x in rule){
        ...
        switch(x){
            ...
            case 'length':
                if(!empty && val.length !== rule_val){
                    msgDiv.innerHTML = name+' should be '+rule_val+ ' characters';
                    msgDiv.style.display = 'block';
                    return false;
                }
                msgDiv.innerHTML = '';
...
function instantValidate(elementId,rule={},msgDivId, len=3){
    let empty;
    let val = (elementId.value).trim();
    let msgDiv = msgDivId;
    let inputLen = val.length;
    if(inputLen > 0){
        empty = false;
    }else{
        empty = true;
